...
ORDER BY sales_amount DESC, sales_quantity DESC, order_count DESC, p.product_name ASC;
```
...
```sql
SELECT * FROM pg_stat_user_indexes WHERE relname IN ('sales_orders', 'sales_order_items');
SELECT * FROM pg_indexes WHERE tablename IN ('sales_orders', 'sales_order_items');
```
...
```sql
CREATE INDEX CONCURRENTLY idx_sales_orders_order_date
    ON sales_orders(order_date);
CREATE INDEX CONCURRENTLY idx_sales_order_items_order_id
    ON sales_order_items(order_id);
CREATE INDEX CONCURRENTLY idx_sales_order_items_product_id
