...
WHERE sa.sale_date BETWEEN '2023-01-01' AND '2023-12-31'
GROUP BY c.category_name, p.brand, s.store_name, s.city,
         EXTRACT(YEAR FROM sa.sale_date), EXTRACT(MONTH FROM sa.sale_date)
ORDER BY c.category_name, p.brand, s.store_name, sale_year, sale_month;
```
...
CREATE INDEX CONCURRENTLY idx_sales_product_store ON sales(product_id, store_id, sale_date);
```
...
```sql
CREATE MATERIALIZED VIEW sales_monthly_report AS
SELECT c.category_name, p.brand, s.store_name, s.city,
       EXTRACT(YEAR FROM sa.sale_date)::INT  AS sale_year,
       EXTRACT(MONTH FROM sa.sale_date)::INT AS sale_month,
       COUNT(*)          AS sale_count,
       SUM(sa.amount)    AS total_amount,
...
CREATE INDEX ON sales_monthly_report(sale_year, sale_month);

-- 定期刷新
REFRESH MATERIALIZED VIEW CONCURRENTLY sales_monthly_report;
```
