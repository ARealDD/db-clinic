...
WITH RECURSIVE sub_departments AS (
    ...
    FROM departments d
    JOIN sub_departments sd ON d.parent_dept_id = sd.dept_id
    WHERE sd.level < 15
)
...
SELECT indexrelname, idx_scan FROM pg_stat_user_indexes WHERE tablename = 'departments';
SELECT seq_scan, idx_scan FROM pg_stat_user_tables WHERE relname = 'departments';
```
...
-- 初始化路径（根节点）
UPDATE departments SET path = dept_id::TEXT WHERE parent_dept_id IS NULL;
...
WITH RECURSIVE build_path AS (
    ...
    FROM departments d
    JOIN build_path bp ON d.parent_dept_id = bp.dept_id
)
UPDATE departments d SET path = bp.path
FROM build_path bp WHERE d.dept_id = bp.dept_id;

-- 为 path 字段创建索引（支持前缀 LIKE）
CREATE INDEX CONCURRENTLY idx_departments_path ON departments(path text_pattern_ops);

-- 查询子部门（无需递归，毫秒级响应）
SELECT * FROM departments
WHERE path LIKE (SELECT path FROM departments WHERE dept_id = 1) || '.%'
ORDER BY path;
```
...
CREATE INDEX ON department_hierarchy(dept_id);
CREATE INDEX ON department_hierarchy(parent_dept_id);
CREATE INDEX ON department_hierarchy(path text_pattern_ops);

-- 组织架构变更时刷新
REFRESH MATERIALIZED VIEW department_hierarchy;
```
