...
-- 手机号同样问题
EXPLAIN ANALYZE
SELECT user_id, username, phone FROM users WHERE phone = 13812345678;   -- 失效
EXPLAIN ANALYZE
SELECT user_id, username, phone FROM users WHERE phone = '13812345678'; -- 有效
```
...
-- 验证 idx_scan 开始增长
SELECT indexrelname, idx_scan FROM pg_stat_user_indexes WHERE tablename = 'users';
```
...
PREPARE get_user_by_id(VARCHAR) AS
