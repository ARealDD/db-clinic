...
SELECT now() AS current_time, pg_current_xlog_location() AS current_lsn;
```
...
-- 为 WHERE 条件字段添加索引
CREATE INDEX CONCURRENTLY idx_products_stock_last_update
    ON products(stock, last_update_time);
```
