...
**现象**：`pg_stat_activity` 中大量 UPDATE 语句的 wait_status 为 'wait acquire lock'，被同一行的其他事务阻塞。

**现象分析**：Lock 等待说明存在热点行竞争，需找出持锁事务和被阻塞事务的关系链。

```sql
-- 查看锁等待情况
SELECT pid, state, wait_status,
       now() - query_start AS duration, query
FROM pg_stat_activity
WHERE wait_status = 'wait acquire lock'
...
JOIN pg_stat_activity blocker
    ON blocked.wait_status = 'wait acquire lock'
...
WHERE l.relation::regclass::text IN ('inventory', 'orders')
