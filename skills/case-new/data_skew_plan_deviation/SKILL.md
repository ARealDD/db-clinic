...
FROM <target_table> p
JOIN <target_table_2> sr ON p.product_id = sr.product_id
...
FROM <target_table> p
JOIN <join_table_1> sr ON p.product_id = sr.product_id
GROUP BY p.category_id;

CREATE INDEX <index_name> category_sales_summary(<filter_col>);
CREATE INDEX <index_name> category_sales_summary(<filter_col>);
