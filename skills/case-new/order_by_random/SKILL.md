...
FROM <target_table> TABLESAMPLE SYSTEM(5)  -- 采样约 5% 的数据块
WHERE category_id = 101 AND stock > 0
LIMIT 10;

-- BERNOULLI 方法：行级采样，随机性更好（速度稍慢）
SELECT product_id, product_name, price, stock
FROM <target_table> TABLESAMPLE BERNOULLI(1)  -- 每行有 1% 概率被选中
WHERE category_id = 101 AND stock > 0
LIMIT 10;
```
...
WITH id_range AS (
    SELECT MIN(<filter_col>) AS min_id, MAX(<filter_col>) AS max_id
    FROM <target_table>
    WHERE category_id = 101 AND stock > 0
),
random_ids AS (
    SELECT FLOOR(RANDOM() * (max_id - min_id + 1) + min_id)::BIGINT AS random_id
    FROM <target_table_2>, generate_series(1, 30)  -- 生成 30 个候选 ID（确保有足够命中）
)
SELECT p.product_id, p.product_name, p.price, p.stock
FROM <target_table> p
JOIN <join_table_2> r ON p.product_id = r.random_id
...
-- 定期（如每小时）批量刷新随机值
UPDATE <target_table> SET random_value = RANDOM();

-- 使用随机值范围查询（利用索引）
WITH pivot AS (SELECT RANDOM() AS r)
SELECT product_id, product_name, price, stock
FROM <target_table>, pivot
...
-- 或使用物化视图（定期刷新）
CREATE MATERIALIZED VIEW <mv_name> NOT EXISTS mv_random_products AS
SELECT product_id, product_name, price, stock, category_id,
       RANDOM() AS random_sort
FROM <target_table>
WHERE stock > 0;

CREATE INDEX <index_name> mv_random_products(<filter_col>, <groupby_col>);
REFRESH MATERIALIZED VIEW <mv_name>;  -- 定期执行
