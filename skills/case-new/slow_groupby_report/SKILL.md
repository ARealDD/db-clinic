...
**现象**：目标表缺少 `<filter_col>` 或复合索引（如 `<filter_col>, <groupby_col>, <join_col>`），导致全表扫描后聚合。
...
```sql
-- 为过滤字段添加索引
CREATE INDEX <index_name> <idx_filter_col> ON <target_table>(<filter_col>);

-- 复合索引覆盖 WHERE + GROUP BY
CREATE INDEX <index_name> <idx_filter_group_join_cols>
    ON <target_table>(<filter_col>, <groupby_col>, <join_col>);

-- 确保 JOIN 字段有索引
CREATE INDEX <index_name> <idx_target_join_col> ON <target_table>(<join_col>);
CREATE INDEX <index_name> <idx_join_table_join_col> ON <join_table>(<join_col>);
```
...
```sql
SET work_mem = '<recommended_work_mem>';

-- 先过滤再 JOIN（减少聚合数据量）
WITH filtered_data AS (
    <filtered_subquery_based_on_slow_sql>
)
<rewritten_sql_based_on_slow_sql>
```
...
<pre_aggregated_query_based_on_slow_sql>;

CREATE INDEX <index_name> <mv_name>(<filter_or_date_col>);
