...
CREATE INDEX CONCURRENTLY <index_name> ON <target_table>(<filter_col>, <groupby_col>) WHERE status = 'pending';
```
...
```sql
SET enable_nestloop = off;  -- test in session only
SET join_collapse_limit = 1;  -- force explicit join <target_table>
```
...
```sql
-- Change: WHERE user_id = '12345' TO: WHERE user_id = 12345
CREATE INDEX <index_name> ON <target_table> ((user_id::text));
```
...
```sql
SELECT ... FROM <target_table> WHERE status = 'pending' LIMIT 100 OFFSET 0;
```
