...
-- SELECT * FROM <target_table>('idx_category_id');
```
...
-- 查看清理效果
SELECT relname, n_dead_tup, last_vacuum FROM pg_stat_user_tables WHERE relname = '<target_table>';
...
-- 验证配置
SELECT reloptions FROM pg_class WHERE relname = '<target_table>';

-- 手动触发一次 autovacuum 检测
SELECT schemaname, tablename, last_autovacuum FROM pg_stat_user_tables WHERE relname = '<target_table>';
```
