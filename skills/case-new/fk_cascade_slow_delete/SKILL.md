...
WHERE tablename IN ('<target_table_1>', '<target_table_2>');

SELECT relname, seq_scan, idx_scan
FROM pg_stat_user_tables
WHERE relname IN ('<target_table_1>', '<target_table_2>');
```
...
```sql
SELECT COUNT(*) FROM <target_table> WHERE project_id = 123;
SELECT COUNT(*) FROM <target_table_2>
WHERE task_id IN (SELECT task_id FROM <target_table> WHERE project_id = 123);
```
...
WHERE task_id IN (SELECT task_id FROM <target_table_2> WHERE project_id = 123)
LIMIT 10000;
-- 重复执行直到影响行数为 0，每批提交
...
-- 2. 删中间层
DELETE FROM <target_table_2> WHERE project_id = 123;

-- 3. 删父表
DELETE FROM <target_table_3> WHERE project_id = 123;
...
-- 软删除操作（瞬间完成）
UPDATE <target_table> SET is_deleted = TRUE WHERE project_id = 123;
UPDATE <target_table>    SET is_deleted = TRUE WHERE project_id = 123;

-- 查询时过滤已删除数据
SELECT * FROM <target_table> WHERE is_deleted = FALSE;

-- 低峰期定期物理清理（分批）
DELETE FROM <target_table_2> WHERE is_deleted = TRUE LIMIT 50000;
DELETE FROM <target_table_3>        WHERE is_deleted = TRUE LIMIT 50000;
DELETE FROM <target_table>     WHERE is_deleted = TRUE LIMIT 50000;
```
